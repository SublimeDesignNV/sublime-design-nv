...
import { CITIES, MATERIALS } from "@/lib/facets.config";
...
export default async function CityServiceMaterialPage({ params }: Props) {
  ...
  const projects = (await listProjectsIndex(500)).filter(
    (project) =>
      project.citySlug === params.city &&
      project.serviceSlug === params.service &&
      project.materialSlug === params.material,
  );
          ...
          <article key={project.slug} style={{ border: "1px solid #ddd", borderRadius: 10, overflow: "hidden" }}>
            ...
            <div style={{ padding: 12 }}>
              ...
              <p style={{ margin: 0, color: "#666", fontSize: 13 }}>
                {[project.roomLabel, project.materialLabel, project.year]
                  .filter(Boolean)
                  .join(" • ")}
              </p>
            </div>
