...
export default async function ServiceRoomPage({ params }: Props) {
  ...
  const projects = serviceProjects.filter((project) => project.roomSlug === params.room);
  ...
  const otherRoomLinks = ROOMS.filter((room) => room.slug !== params.room)
    .map((room) => {
      const matches = serviceProjects.filter((project) => project.roomSlug === room.slug).length;
      return {
    ...
    <main style={{ padding: 40 }}>
      ...
      <FacetBreadcrumbs
        crumbs={[
          ...
          { label: labels.roomLabel, href: `/services/${params.service}/room/${params.room}` },
        ]}
      />
      ...
      <RelatedFacetLinks title="Other Rooms" links={otherRoomLinks} />
      <RelatedFacetLinks title="Materials" links={materialLinks} />
          ...
          <article key={project.slug} style={{ border: "1px solid #ddd", borderRadius: 10, overflow: "hidden" }}>
            ...
            <div style={{ padding: 12 }}>
              ...
              <p style={{ margin: 0, color: "#666", fontSize: 13 }}>
                {[project.cityLabel, project.state, materialLabelFromSlug(project.materialSlug || ""), project.year]
                  .filter(Boolean)
                  .join(" • ")}
              </p>
            </div>
